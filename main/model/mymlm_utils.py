...
class MyDataCollatorForT5MLM:
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> BatchEncoding:
        ...
        for example_i in examples:
            ...
            if all(label != -100 for label in example_i['labels']):
                # print('1')
        ...
        if len(examples_mlm) != 0:
            ...
            #     # If not, create an attention mask  which shape is the same as batch['input_ids']
            #     batch["attention_mask"] = np.ones_like(batch["input_ids"])
        if len(examples_src_tgt) != 0:
            ...
            # Check if 'attention_mask' is in the batch
            # if "attention_mask" not in batch_src_tgt:
            #     # If not, create an attention mask
            #     batch_src_tgt["attention_mask"] = np.ones_like(batch_src_tgt["input_ids"])
...
def tokenize_function(examples, tokenizer, in_length):
    ...
    labels = np.full_like(concatenated_ids, -100)
    result = {"input_ids": concatenated_ids, "labels": labels}
...
def process_dataset(dataset_splits, args, tokenizer):
    ...
    for split, dataset_split in dataset_splits.items():
        dataset_split = dataset_split.map(
            ...
            fn_kwargs={
                'tokenizer': tokenizer,
                'in_length': args.before_mask_input_length,
            },
            remove_columns=['text'],
        )

        dataset_split = dataset_split.shuffle(seed=1234)
